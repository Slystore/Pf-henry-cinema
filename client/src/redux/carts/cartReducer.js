...
function cartReducer(state = initialState, action) {
    ...
    switch (action.type) {
        ...
        case CINEMAS: {
            let cinePrueba= state.postCart.length ===1? {...state.postCart[0], cinemaId:action.payload}:
            state.postCart.map(item => item.cinemaId? {item}:{...item, cinemaId:action.payload})
            return{
                ...state,
